             ...
             foreach (string file in files)
            {
                ...
                else if (loadtype == LoadType.PART)
                {
                    for (int i = 0; i < added.Count; i++)
                    {
                        ...
                        // Controllo di sicurezza per evitare il crash se l'array è troppo corto
                        // if (QtyJob != null && i < QtyJob.Length)
                        // {
                        //     MessageBox.Show($"Ciclo indiceGlobaleParti = {i}\nValore QtyJob[{indiceGlobaleParti}] = {QtyJob[indiceGlobaleParti]}");
                        // }
                        // else
                        // {
                        //     MessageBox.Show($"Ciclo i = {i}\nAttenzione: QtyJob non ha un elemento per l'indice {i}!");
                        // }
