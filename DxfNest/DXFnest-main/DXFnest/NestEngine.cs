...
namespace DXFnest
{
    public class NestEngine
    {
        ...
        // aggiunto 01/06/2026 ********************************************************************************************
        public void EasyCutLoadDxfs(List<string> files, LoadType loadtype, int[] QtyJob)
        {

            int indiceGlobaleParti = 0;
                ...
                else if (loadtype == LoadType.PART)
                {
                    for (int i = 0; i < added.Count; i++)
                    {
                        PartItems.Add(new PartItem
                        {
                            ...
                            UsedQty = 0,
                            // IniQty = 1,
                            IniQty = (QtyJob != null && indiceGlobaleParti < QtyJob.Length) ? QtyJob[indiceGlobaleParti] : 1,
                            Features = added[i],
                            SourceFileName = file,
                        });
            ...
            if (Context.Nest.nests != null)
            {
                ...
                foreach (SheetPlacementItem nest in result.placements.First())
                {
                    NestItems.Add(new NestItem
                    {
                        ...
                        Name = "EasyCutNest_" + NestItems.Count,
        ...
        public void Export(object obj, string file)
        {
            ...
            if (obj.GetType() == typeof(NestItem))
            {
                ...
                if (NestItems.Any())
                {
                    ...
                    // aggiunto 01/06/2026 ************************************************************************
                    // se la lamiera è importata da dxf non serve questo passaggio
                    //       var currentSheet = SheetItems[nest.SheetSource];
                    // CREA UN LAYER DEDICATO ALLA LAMIERA (SE NON ESISTE GIÀ)
                    //       string sheetLayerName = "LAMIERE_FOGLIO";
                    //       Layer sheetLay = doc.Layers.ToList().Find(l => l.Name == sheetLayerName);
                    //       if (sheetLay == null)
                    //       {
                    //           doc.Layers.Add(new Layer(sheetLayerName));
                    //           sheetLay = doc.Layers.Last();
                    //       }
                    // CREA IL RETTANGOLO DELLA LAMIERA CON UNA LWPOLYLINE
                    //       LwPolyline sheetOutline = new LwPolyline
                    //       {
                    //           Layer = sheetLay,
                    //           IsClosed = true
                    //       };
                    // NOTA: Verifica se la tua classe SheetItem usa 'Width' e 'Height' 
                    // o proprietà simili come 'DimX' e 'DimY' per memorizzare le dimensioni.
                    //       double sheetW = currentSheet.LX;
                    //       double sheetH = currentSheet.LY;
                    // AGGIUNGE I 4 VERTICI DEL PERIMETRO (PARTENDO DA 0,0)
                    //       sheetOutline.Vertices.Add(new LwPolyline.Vertex(new XY(0, 0)));
                    //       sheetOutline.Vertices.Add(new LwPolyline.Vertex(new XY(sheetW, 0)));
                    //       sheetOutline.Vertices.Add(new LwPolyline.Vertex(new XY(sheetW, sheetH)));
                    //       sheetOutline.Vertices.Add(new LwPolyline.Vertex(new XY(0, sheetH)));
                    // INIETTA IL RETTANGOLO NEL DOCUMENTO DXF
                    //       doc.ModelSpace.Entities.Add(sheetOutline);
                    // fine modifica ************************************************************************
    ...
    public class Options
    {
       ...
       // [DisplayName("NEST ARC SEGMENTS MAX LENGTH")]
        [DisplayName("DISCRETIZZA ARCHI MAX LUNGHEZZA")]
        //[TypeConverter(typeof(PositiveDoubleTypeConverter))]
        public double NestArcSegmentsMaxLength { get; set; } = 250.0;

        [Category("DXF Interchange")]
        [DisplayName("DXF PARTI")]
        //public string DxfSourceShape { get; set; } = (string)Registry.GetValue(@"HKEY_CURRENT_USER\Software\EasyCut", "DxfNestSourceShape", "Null");
        public string DxfSourceShape { get; set; } = "-";

        [Category("DXF Interchange")]
        [DisplayName("DXF LAMIERE")]
        //public string DxfSouceSheet { get; set; } = (string)Registry.GetValue(@"HKEY_CURRENT_USER\Software\EasyCut", "DxfNestSourceSheet", "Null");
        public string DxfSourceSheet { get; set; } = "-";

        [Category("DXF Interchange")]
        [DisplayName("DXF NESTING")]
        //public string DxfSouceSheet { get; set; } = (string)Registry.GetValue(@"HKEY_CURRENT_USER\Software\EasyCut", "DxfNestSourceSheet", "Null");
        public string DxfNesting { get; set; } = "-";
    }
    ...
    public class PartItem
    {
        ...
        [DisplayName("QTY NEST")]
        public int UsedQty { get; set; }

        //[DisplayName("INITAL QTY")]
        [DisplayName("QTY INIZIALI")]
        //[TypeConverter(typeof(PositiveIntegerTypeConverter))]
    ...
    public class SheetItem
    {
        ...
        [DisplayName("LUNGHEZZA X")]
        //[TypeConverter(typeof(PositiveDoubleTypeConverter))]
        public double LX { get; set; }

        //[DisplayName("WIDTH Y")]
        [DisplayName("ALTEZZA Y")]
        //[TypeConverter(typeof(PositiveDoubleTypeConverter))]
